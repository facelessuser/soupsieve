...
class CSSStrainer:
    ...
    def __init__(self, sieve, document):
        """Initialize."""
    ...
    def get_attribute_by_name(self, attrs, name, default=None):
        ...
        if self.is_xml:
            try:
                value = _DocumentNav.normalize_value(attrs[name])
            except KeyError:
                pass
        else:
            for k, v in attrs.items():
                if util.lower(k) == name:
                    value = _DocumentNav.normalize_value(v)
                    break
        return value
    ...
    def match_id(self, attrs, ids):
        ...
        found = True
        for i in ids:
            if i != self.get_attribute_by_name(attrs, 'id', ''):
                found = False
                break
        return found
    ...
    def get_classes(self, attrs):
        ...
        classes = self.get_attribute_by_name(attrs, 'class', [])
        if isinstance(classes, str):
            classes = RE_NOT_WS.findall(classes)
        return classes
    ...
    def match_attributes(self, attrs, attributes):
        ...
        if attributes:
            for a in attributes:
                value = self.match_attribute_name(attrs, a.attribute, a.prefix)
                pattern = a.xml_type_pattern if self.is_xml and a.xml_type_pattern else a.pattern
                if isinstance(value, list):
                    value = ' '.join(value)
                if value is None:
    ...
    def strain(self, name, attrs=None):
        ...
        if isinstance(name, bs4.Tag):
            return self.sieve.match(name)
        else:
            # Tree builder which has limited CSS support
            tag_name = name
            tag_attrs = attrs
            return self.match_selectors(tag_name, tag_attrs, self.sieve.selectors)
...
class SoupSieve(ct.Immutable):
    ...
    def strainer(self, document='html'):
        ...
        return CSSStrainer(self, document).strain
